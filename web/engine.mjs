...
import { engineLock } from './engine-lock.mjs';
...
export async function boot({ mode = 't0', imageURL, imageSha256, localEngine = false, signal, onData = () => {}, onStatus = () => {}, onError = () => {}, onProgress }) {
  ...
  const engineRoot = localEngine ? new URL('./vendor/qemu/', import.meta.url) : new URL(engineLock.baseURL);
