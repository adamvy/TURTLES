...
let manifest, bootImage, selectedMode = 'js', runningMode, runningRuntime, frame, booting = false, ready = false;
...
function boot() {
  ...
  runningMode = selectedMode; runningRuntime = runtime(); booting = true; tail = '';
...
function appendUART(text) {
  ...
  if (/(?:^|[\r\n])HALTED: (?:value heap exhausted|compiler\/frame arena exhausted|data stack exhausted|native call stack exhausted|initialization failed)[^\r\n]*\r?\n$/.test(tail)) {
    ...
    status(`${runningMode === 'js' ? 'JS-like' : 'Raw T0'} running on ${runningRuntime === 'arm' ? 'ARM · QEMU' : 'the JavaScript host'}`, 'running');
    $('#progress-wrap').hidden = true;
  }
  controls();
}
addEventListener('message', event => {
  ...
  if (message?.type === 'ready') {
    frame.contentWindow.postMessage({type: 'boot', runtime: runningRuntime, mode: runningMode,
...
try {
  ...
  controls();
} catch (error) { status('ARM images unavailable', 'error'); notice(`${error.message} You can still select the JavaScript host.`); controls(); }
