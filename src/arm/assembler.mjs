...
function reg(text) {
  ...
  if (t === 'sp' || t === 'wsp' || t === 'xzr' || t === 'wzr') return { n: 31, bits: t[0] === 'w' ? 32 : 64, sp: t.endsWith('sp') };
  const m = /^([xw])(\d+)$/.exec(t || '');
  if (!m || Number(m[2]) > 30) throw Error(`invalid register: ${text}`);
  return { n: Number(m[2]), bits: m[1] === 'x' ? 64 : 32, sp: false };
}
function isReg(text) { return /^(?:[xw]\d+|sp|wsp|xzr|wzr)$/i.test(text || ''); }
...
function sameWidth(...registers) {
  if (registers.some(r => r.bits !== registers[0].bits)) throw Error('register widths do not match');
}
function noSP(registers) {
  if (registers.some(r => r.sp)) throw Error('SP is not supported by this instruction');
}
...
function encode(node, evaluate) {
  ...
  if (op === 'mov') {
    const d = r(0);
    if (!isReg(a[1])) { if (d.sp) throw Error('immediate mov requires a general register'); return moveWords(d, imm(a[1]), node.fixedMove ? d.bits / 16 : undefined); }
    const n = r(1); sameWidth(d, n);
  ...
  if (['add', 'sub', 'adds', 'subs', 'cmp', 'cmn'].includes(op)) {
    ...
    if (isReg(operand)) {
      const m = reg(operand); sameWidth(d, m); noSP([m]);
  ...
  if (['mul', 'madd', 'msub', 'sdiv', 'udiv'].includes(op)) {
    const d = r(0), n = r(1), m = r(2); sameWidth(d, n, m); noSP([d, n, m]);
    const base = { mul: 0x1b007c00, madd: 0x1b000000, msub: 0x1b008000, sdiv: 0x1ac00c00, udiv: 0x1ac00800 }[op];
    const accumulator = op === 'msub' || op === 'madd' ? r(3) : null; if (accumulator) { sameWidth(d, accumulator); noSP([accumulator]); }
  ...
  if (['and', 'ands', 'orr', 'eor', 'tst'].includes(op)) {
    ...
    const d = test ? { ...r(0), n: 31 } : r(0), n = r(test ? 0 : 1), operand = a[test ? 1 : 2]; sameWidth(d, n); noSP([d, n]);
    if (a.length !== (test ? 2 : 3)) throw Error('logical register shifts are not supported');
    const opc = { and: 0, orr: 1, eor: 2, ands: 3 }[actual] << 29;
    if (isReg(operand)) { const m = reg(operand); sameWidth(d, m); noSP([m]); return emit((sf(d) | opc | 0x0a000000) + (m.n << 16) + (n.n << 5) + d.n); }
    return emit((sf(d) | opc | 0x12000000) + logicalImmediate(imm(operand), d.bits) + (n.n << 5) + d.n);
  }
  if (['lsl', 'lsr', 'asr'].includes(op)) {
    const d = r(0), n = r(1); sameWidth(d, n); noSP([d, n]);
    if (isReg(a[2])) { const m = r(2); sameWidth(d, m); noSP([m]); return emit((sf(d) | { lsl: 0x1ac02000, lsr: 0x1ac02400, asr: 0x1ac02800 }[op]) + (m.n << 16) + (n.n << 5) + d.n); }
  ...
  if (op === 'neg' || op === 'mvn') { const d = r(0), n = r(1); sameWidth(d, n); noSP([d, n]); return emit((sf(d) | (op === 'neg' ? 0x4b0003e0 : 0x2a2003e0)) + (n.n << 16) + d.n); }
  if (op === 'adr' || op === 'adrp') {
    const d = r(0); if (d.bits !== 64 || d.sp) throw Error('adr requires X register');
  ...
  if (op === 'cbz' || op === 'cbnz') { noSP([r(0)]); return emit((sf(r(0)) | (op === 'cbz' ? 0x34000000 : 0x35000000)) + (relative(a[1], 19) << 5) + r(0).n); }
  if (op === 'tbz' || op === 'tbnz') { const n = r(0), bit = range(imm(a[1]), n.bits === 64 ? 6 : 5); noSP([n]); return emit((op === 'tbz' ? 0x36000000 : 0x37000000) + ((bit >>> 5) * 0x80000000) + ((bit & 31) << 19) + (relative(a[2], 14) << 5) + n.n); }
  if (['br', 'blr', 'ret'].includes(op)) { const n = a[0] ? r(0) : { n: 30, bits: 64 }; if (n.bits !== 64 || n.sp) throw Error('branch requires X register'); return emit({ br: 0xd61f0000, blr: 0xd63f0000, ret: 0xd65f0000 }[op] + (n.n << 5)); }
  if (['ldr', 'str', 'ldrb', 'strb', 'ldrh', 'strh', 'ldrsw', 'ldur', 'stur', 'ldurb', 'sturb'].includes(op)) {
    ...
    if (signWord && (d.bits !== 64)) throw Error('ldrsw requires X register');
    const scale = byte ? 1 : half ? 2 : signWord ? 4 : d.bits / 8;
    if ((byte || half) && (d.bits !== 32)) throw Error('byte/halfword access requires W register');
    const match = /^\[(.+)\](!?)$/.exec(a[1]); if (!match) throw Error('invalid load/store address');
    const fields = splitOperands(match[1]), n = reg(fields[0]); if (n.bits !== 64 || (n.n === 31 && !n.sp)) throw Error('address base requires X register or SP');
    const size = Math.log2(scale), opc = signWord ? 0x800000 : load ? 0x400000 : 0;
    if (fields[1] && isReg(fields[1])) {
      const m = reg(fields[1]); if (m.bits !== 64 || m.sp) throw Error('index requires X register');
      ...
      return emit((size * 0x40000000 + opc + 0x38206800) + (m.n << 16) + (shift ? 1 << 12 : 0) + (n.n << 5) + d.n);
    }
    ...
    if (!pre && !post && !op.includes('ur') && offset >= 0n && offset % BigInt(scale) === 0n) return emit(size * 0x40000000 + opc + 0x39000000 + (range(offset / BigInt(scale), 12) << 10) + (n.n << 5) + d.n);
    return emit(size * 0x40000000 + opc + 0x38000000 + (range(offset, 9, true) << 12) + (pre ? 0xc00 : post ? 0x400 : 0) + (n.n << 5) + d.n);
  }
  if (op === 'stp' || op === 'ldp') {
    ...
    return emit(sf(d) + 0x28000000 + mode + (op === 'ldp' ? 1 << 22 : 0) + (range(offset / scale, 7, true) << 15) + (t.n << 10) + (n.n << 5) + d.n);
  }
  if (op === 'csel') { const d = r(0), n = r(1), m = r(2); sameWidth(d, n, m); noSP([d, n, m]); return emit((sf(d) | 0x1a800000) + (m.n << 16) + (condition(a[3]) << 12) + (n.n << 5) + d.n); }
  if (op === 'cset') { noSP([r(0)]); return emit((sf(r(0)) | 0x1a9f07e0) + ((condition(a[1]) ^ 1) << 12) + r(0).n); }
  ...
  if (op === 'mrs' || op === 'msr') {
    ...
    const n = r(op === 'mrs' ? 0 : 1); if (n.bits !== 64 || n.sp) throw Error('system register access requires X register');
    return emit((op === 'mrs' ? 0xd5380000 : 0xd5180000) + systemRegisters[name] + n.n);
  }
  if (op === 'clz') { const d = r(0), n = r(1); sameWidth(d, n); noSP([d, n]); return emit((sf(d) | 0x5ac01000) + (n.n << 5) + d.n); }
