...
// Native bridge for the upstream JS-like frontend. Parsing and translation
// still run in guest T0 (`jsCompile`). Unlike the upstream JS hook, generated
// T0 is emitted into the active builder, so local references and side effects
// occur when the surrounding function executes, not while it is compiled.
core_compile_js:
    stp x19, x20, [sp, #-64]!
    stp x21, x25, [sp, #16]
    stp x26, x30, [sp, #32]
